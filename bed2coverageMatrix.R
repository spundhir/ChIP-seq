...
option_list <- list(
	...
	make_option(c("-o", "--matrixFile"), help="output matrix file"),
...
if(is.null(opt$bedFile) | is.null(opt$bamFile) | is.null(opt$matrixFile)) {
...
if(file.exists(sprintf("%s.session", opt$matrixFile))) {
    coveragePlot <- opt$coveragePlot
    load(sprintf("%s.session", opt$matrixFile))
    ...
    save.session(sprintf("%s.session", opt$matrixFile))
...
write.table(cov.mat.combined, file = opt$matrixFile, sep="\t", quote = F, col.names = T, row.names = T)

if(!is.null(opt$coveragePlot)) {
    draw.heatmap(data=cov.mat.list,outfile=sprintf("%s.png", opt$matrixFile))
}
