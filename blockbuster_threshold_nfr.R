...
if(is.null(ARGV[4])) {
    ...
    ggsave(g, dpi=300, height=15, width=15, filename=ARGV[3], useDingbats=FALSE)
} else {
    ...
    if(file.exists(ARGV[3])) {
        ...
        ggsave(g, dpi=300, filename=ARGV[4], useDingbats=FALSE)
        ...
        histone_percentile_rep1 <- round(log(quantile(histone$V1, probs=0.0005, type=8)), digits=2)
        ...
        histone_percentile_rep2 <- round(log(quantile(histone$V2, probs=0.0005, type=8)), digits=2)
