...
@data=`less $outDir/$ID.All.nfr$fileSuffix`;

open(UCSCFILE, ">$outDir/$ID.All.nfr.ucsc$fileSuffix") || die $!;
print UCSCFILE "track name=\"Predicted NFR ($ID.All.nfr$fileSuffix)\" description=\"Predicted NFR ($ID.All.nfr$fileSuffix)\" itemRgb=\"On\"\n";

foreach my $l(@data) {
    my @F=split(/\s+/,$l);
    my @startBlock=split(/[\:\-]+/,$F[6]);
    my @endBlock=split(/[\:\-]+/,$F[7]);
    print UCSCFILE "$startBlock[0]\t$startBlock[1]\t$startBlock[2]\t$F[6]\t$F[8]\t.\t$startBlock[1]\t$startBlock[2]\t0,255,0\n";
    print UCSCFILE "$F[0]\t$F[1]\t$F[2]\t$F[3]\t$F[4]\t$F[5]\t$F[1]\t$F[2]\t255,0,0\n";
    print UCSCFILE "$endBlock[0]\t$endBlock[1]\t$endBlock[2]\t$F[7]\t$F[10]\t.\t$endBlock[1]\t$endBlock[2]\t0,255,0\n";
}

close UCSCFILE;

## create output file for writing UCSC tracks of NFR (significant)
@data=`less $outDir/$ID.All.nfr.sig$fileSuffix`;

open(UCSCFILE, ">$outDir/$ID.All.nfr.sig.ucsc$fileSuffix") || die $!;
print UCSCFILE "track name=\"Predicted NFR ($ID.All.nfr$fileSuffix)\" description=\"Predicted NFR ($ID.All.nfr$fileSuffix)\" itemRgb=\"On\"\n";
